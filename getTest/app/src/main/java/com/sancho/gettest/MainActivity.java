...
public class MainActivity extends ActionBarActivity {
    ...
    public void jsonCallback(String URL, JSONArray json, AjaxStatus status) {
        ...
        Log.d("LOG_null","NULL or not NULL");
        if (json != null) {
        ...
        try {
            ...
            for (int i = 0; i < arr.length(); i++) {
                ...
                try {
                    ...
                    mDatabaseHelper.addItem(item.getInt("id"), item.getString("title"), parent, level);


                } catch (JSONException e) {
                    e.printStackTrace();
                    mDatabaseHelper.addItem(item.getString("title"), parent, level);
